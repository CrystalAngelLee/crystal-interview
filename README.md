...
33. [下面代码输出什么](./javascript/31_下面代码输出什么.md)
...
34. [用正则表达式实现分割数字 “12345678901” 每三个以一个逗号 “ ，” 划分](./javascript/用正则表达式实现分割数字 "12345678901" 每三个以一个逗号 " ，" 划分.md)

35. [实现数组去重元素并使元素降序排列](./javascript/实现数组去重元素并使元素降序排列.md)

36. [事件委托（手写例子）事件冒泡和捕获，如何阻止冒泡？如何阻止默认事件](./javascript/事件委托（手写例子）事件冒泡和捕获，如何阻止冒泡？如何阻止默认事件)

37. [ES6中箭头有哪些新特点？并说明箭头函数和普通函数中的this指向](./javascript/ES6中箭头有哪些新特点？并说明箭头函数和普通函数中的this指向.md)

38. [var let const 的理解](./javascript/var let const 的理解.md)

39. [实现一个instanceof](./javascript/instanceof.md)

40. [说一说闭包](./javascript/闭包.md)

41. 代码执行过程中的内存分配

42. [创建ajax的过程](./javascript/ajax/创建ajax的过程.md)

43. [ajax 的缺点和在 IE 下的问题](./javascript/ajax/ajax 的缺点和在 IE 下的问题.md)

44. [如何实现 ajax 请求，假如我有多个请求，我需要让这些 ajax 请求按照某种顺序一次执行，有什么办法呢？如何处理 ajax 跨域 ](./javascript/ajax/如何实现 ajax 请求，假如我有多个请求，我需要让这些 ajax 请求按照某种顺序一次执行，有什么办法呢？如何处理 ajax 跨域 .md)

45. [如何实现一个 ajax 请求？如果我想发出两个有顺序的 ajax 需要怎么做](./javascript/ajax/如何实现 ajax 请求，假如我有多个请求，我需要让这些 ajax 请求按照某种顺序一次执行，有什么办法呢？如何处理 ajax 跨域 .md)

46. [Flash、Ajax 各自的优缺点，在使用中如何取舍？](./javascript/ajax/FlashPKAjax.md)

47. [如何将字符串转化为数字？](./javascript/字符串转数字.md)

48. 拷贝一个很多嵌套的对象怎么实现？

49. [写一个function,清除字符串前后的空格](./javascript/清除字符串前后的空格.js)

50. [不用loop循环，创建一个长度为100的数组，并且每个元素的值等于他的下标](./javascript/不用loop循环，创建一个长度为100的数组，并且每个元素的值等于他的下标.md)

51. [如何实现数组扁平化](./javascript/数组扁平化.md)

52. [数组的去重，尽可能多方法](./javascript/数组去重.md)

53. [闭包实现加法器](./javascript/闭包实现加法器.js)

54. 字符串反转实现

55. [说出数组的方法不改变原数组、改变原数组](./javascript/数组的方法.md)

56. 描述一下map、set方法的特点以及区别
...
61. [null 和 undefined 的区别 ](./javascript/null 和 undefined 的区别 .md)

62. [javascript 对象的几种创建方式 ](./javascript/javascript 对象的几种创建方式 .md)

63. [new 操作符具体干了什么呢? ](./javascript/new操作符具体干了什么呢.md)

64. [call()和 apply() 的区别和作用](./javascript/call_apply_bind区别和作用.md)

65. [说说你对 Promise 的理解](./javascript/Promise 的理解.md)

66. [说说你对 AMD 和 Commonjs 的理解](./javascript/AMD 和 Commonjs 的理解.md)

67. [请解释什么是事件代理](./javascript/事件代理.md)

68. [事件、IE与火狐的事件机制有什么区别？ 如何阻止冒泡？ ](./javascript/事件、IE与火狐的事件机制有什么区别？ 如何阻止冒泡？.md)

69. [异步加载和延迟加载](./javascript/异步加载和延迟加载.md)

70. [defer 和 async ](./javascript/defer和async.md)

71. [js 延迟加载的方式有哪些？]((./javascript/异步加载和延迟加载.md))

72. [栈和堆的区别](./javascript/栈和堆的区别.md)
...
3. [HTML5 的优点与缺点](./html/HTML5 的优点与缺点.md)
4. [回流与重绘](./html/回流与重绘.md)
5. [左边宽度固定，右边自适应](./html/左边宽度固定，右边自适应.html)
6. [label 的作用是什么？是怎么用的?](./html/label.md)
7. [已知ID的input输入框，希望获取这个输入框的输入值，怎么做?](./html/已知ID的input输入框，希望获取这个输入框的输入值.md)
8. [为什么最好把 CSS 的 `<link>` 标签放在 `<head></head>` 之间？为什么最好把 JS 的 `<script>` 标签恰好放在 `</body>` 之前，有例外情况吗？](./html/为什么最好把CSS的link标签放在head之间？为什么最好把JS的script标签最好放在body之后，有例外的情况吗.md)
...
2. [腾讯视频]http2.0的优势
3. [http和https的区别](./http/http和https.md)
4. [说一下协商缓存和强制缓存](./http/协商缓存和强制缓存.md)
5. [如何解决跨域问题](./http/跨域.md)
6. [TCP 和 UDP 的区别](./http/TCP 和 UDP 的区别.md)
7. [什么是 Etag](./http/Etag.md)
8. 关于 Http 2.0 你知道多少
...
1. [浏览器页面有哪三层构成，分别是什么，作用是什么?](./others/浏览器页面.md)
...
5. Jsbridge是如何工作的
6. [滴滴]APP内嵌H5页面如何和APP本身进行通信
7. [滴滴]H5的开发和PC端的开发有什么本质的不同 
8. [滴滴]如何针对H5页面进行远程调试？
9. [滴滴]如何进行手机的页面或者应用的抓包
10. [滴滴]已知公司附近有两个加油站，和一条主干道，要求在24小时内尽可能准确的统计全市有多少台车， 说出你的思路 
11. [小米]说一下从浏览器输入网址到页面渲染中间发生了什么
12. [小米]如何实现一个简单的单点登录
13. [小米]说一下关系数据库和非关系数据库的区别，并说下使用场景
14. [小米]说一下关系数据库外键的使用
15. [小米]**说下你知道的设计模式及应用场景**
16. [小米]如何用缓存进行前端优化；说下浏览器缓存、DNS缓存、nginx缓存、服务端缓存的区别；强缓存和协商缓存的应用
17. [小米]如何开启GPU加速，GPU加速的作用是什么
18. [小米]是否了解浏览器内核相关技术
19. [小米]说一下jsbridge是如何实现的  
20. [完美世界]题目：两个玻璃小球，从一到一百层中间的某一层落下会碎，不考虑小球的磨损情况，比如说一层落下不会碎，小球不会受到任何伤害，如何最快找出小球破碎的最低层
21. [完美世界]扩展，有很多的小球，题目类似，然后把一到一百层放到一个数组中，数组是乱序，如果最快找出破碎的最低层 
22. [完美世界]题目：一个函数，没有入参，如何实现每次调用返回1到200中的任意一个数，且每次返回的数不重复，如果1到200都被返回过了，就不在返回任何数字
23. [完美世界]百度网盘的下载功能如何实现，浏览器环境，不使用浏览器自带的下载功能，可以暂停，可以继续
24. [完美世界]给定一个整数 N，写一个程序判断是否存在 2 个整数 a、b（a < b），使得 a^2 + b^2 = N，如果存在返回所有的结果
